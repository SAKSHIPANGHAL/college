...
document.querySelector('.timetable div h2').innerHTML = "Today's Timetable"; //To prevent overwriting the heading on loading;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Function to update progress circle based on percentage
        function updateProgressCircles(selector) {
            document.querySelectorAll(selector).forEach(progress => {
                const percentage = parseInt(progress.querySelector('.number p').innerText, 10);
                progress.querySelector('circle:nth-child(2)').style.setProperty('--percentage', percentage);
            });
        }

    
        updateProgressCircles('.internship-item .progress');
        updateProgressCircles('.project-item .progress');
    });
